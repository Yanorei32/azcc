...
case NODE_ASSIGN: {
  ...
  //代入は代入先の型を最優先とする
  if (type_compare_deep_with_implicit_cast(node->lhs->type, node->rhs->type)) {
...
case NODE_DOT:
  ...
  if (node->rhs->kind != NODE_VAR)
    ERROR_AT(node->source, "ドット演算子のオペランド型が不正です");
  node->rhs->variable =
      member_container_get(node->lhs->type->members, node->rhs->variable->name);
  tag_type_to_node(node->rhs, context);
